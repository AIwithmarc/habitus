...
(function() {
    ...
    window.PerhapsDebug = {
        // Test adding an idea
        testAddIdea: () => {
            console.log('[PerhapsDebug] Testing add idea...');
            const testIdeas = [
            ...
            console.log('[PerhapsDebug] Adding test idea:', randomIdea);
            
            if (window.Perhaps && window.Perhaps.addIdea) {
                // Temporarily set the input value
                const ideaInput = document.getElementById('ideaInput');
                if (ideaInput) {
                    ideaInput.value = randomIdea;
                    window.Perhaps.addIdea();
                } else {
                    console.error('[PerhapsDebug] Idea input not found');
                }
            } else {
                console.error('[PerhapsDebug] Perhaps module not available');
            }
        },

        // Test converting to goal
        testConvertToGoal: () => {
            console.log('[PerhapsDebug] Testing convert to goal...');
            const ideas = window.Perhaps ? window.Perhaps.getIdeas() : [];
            const activeIdeas = ideas.filter(idea => !idea.archived && !idea.implemented);
            
            if (activeIdeas.length === 0) {
                console.log('[PerhapsDebug] No active ideas to convert');
                alert('No hay ideas activas para convertir. Agrega una idea primero.');
                return;
            }
            
            const firstIdea = activeIdeas[0];
            console.log('[PerhapsDebug] Converting idea to goal:', firstIdea);
            
            if (window.Perhaps && window.Perhaps.convertToGoal) {
                window.Perhaps.convertToGoal(firstIdea.id);
            } else {
                console.error('[PerhapsDebug] convertToGoal function not available');
            }
        },

        // Test converting to task
        testConvertToTask: () => {
            console.log('[PerhapsDebug] Testing convert to task...');
            const ideas = window.Perhaps ? window.Perhaps.getIdeas() : [];
            const activeIdeas = ideas.filter(idea => !idea.archived && !idea.implemented);
            ...
            const firstIdea = activeIdeas[0];
            console.log('[PerhapsDebug] Converting idea to task:', firstIdea);
            
            if (window.Perhaps && window.Perhaps.convertToTask) {
                window.Perhaps.convertToTask(firstIdea.id);
            } else {
                console.error('[PerhapsDebug] convertToTask function not available');
            }
        },

        // Test archiving idea
        testArchiveIdea: () => {
            console.log('[PerhapsDebug] Testing archive idea...');
            const ideas = window.Perhaps ? window.Perhaps.getIdeas() : [];
            const activeIdeas = ideas.filter(idea => !idea.archived && !idea.implemented);
            
            if (activeIdeas.length === 0) {
                console.log('[PerhapsDebug] No active ideas to archive');
                alert('No hay ideas activas para archivar. Agrega una idea primero.');
                return;
            }
            
            const firstIdea = activeIdeas[0];
            console.log('[PerhapsDebug] Archiving idea:', firstIdea);
            
            if (window.Perhaps && window.Perhaps.archiveIdea) {
                window.Perhaps.archiveIdea(firstIdea.id);
            } else {
                console.error('[PerhapsDebug] archiveIdea function not available');
            }
        },

        // Show ideas data
        showIdeasData: () => {
            console.log('[PerhapsDebug] Showing ideas data...');
            const ideas = window.Perhaps ? window.Perhaps.getIdeas() : [];
            const activeIdeas = ideas.filter(idea => !idea.archived && !idea.implemented);
            const archivedIdeas = ideas.filter(idea => idea.archived && !idea.implemented);
            const implementedIdeas = ideas.filter(idea => idea.implemented);
            
            console.log('[PerhapsDebug] Total ideas:', ideas.length);
            console.log('[PerhapsDebug] Active ideas:', activeIdeas.length);
            console.log('[PerhapsDebug] Archived ideas:', archivedIdeas.length);
            console.log('[PerhapsDebug] Implemented ideas:', implementedIdeas.length);
            console.log('[PerhapsDebug] All ideas:', ideas);
            
            alert(`📊 Datos de Ideas:\n\nTotal: ${ideas.length}\nActivas: ${activeIdeas.length}\nArchivadas: ${archivedIdeas.length}\nImplementadas: ${implementedIdeas.length}\n\nRevisa la consola para más detalles.`);
        },

        // Clear all ideas
        clearAllIdeas: () => {
            if (!confirm('¿Estás seguro de que quieres eliminar TODAS las ideas? Esta acción no se puede deshacer.')) {
                return;
            }
            
            console.log('[PerhapsDebug] Clearing all ideas...');
            try {
                localStorage.removeItem('habitus_ideas');
                if (window.Perhaps && window.Perhaps.renderIdeas) {
                    window.Perhaps.renderIdeas();
                }
                console.log('[PerhapsDebug] All ideas cleared');
                alert('Todas las ideas han sido eliminadas.');
            } catch (error) {
                console.error('[PerhapsDebug] Error clearing ideas:', error);
                alert('Error al limpiar las ideas: ' + error.message);
            }
        },

        // Test modules availability
        testModules: () => {
            console.log('[PerhapsDebug] Testing modules availability...');
            
            const modules = {
                'Perhaps': !!window.Perhaps,
                'Goals': !!window.Goals,
                'Tasks': !!window.Tasks,
                'Roles': !!window.Roles,
                'App': !!window.App
            ...
            console.log('[PerhapsDebug] Modules status:', modules);
            
            // Test specific functions
            if (window.Perhaps) {
                console.log('[PerhapsDebug] Perhaps functions:', {
                    'addIdea': typeof window.Perhaps.addIdea,
                    'convertToGoal': typeof window.Perhaps.convertToGoal,
                    'convertToTask': typeof window.Perhaps.convertToTask,
                    'archiveIdea': typeof window.Perhaps.archiveIdea,
                    'getIdeas': typeof window.Perhaps.getIdeas
                });
            }
            
            if (window.Goals) {
                console.log('[PerhapsDebug] Goals functions:', {
                    'addGoal': typeof window.Goals.addGoal,
                    'getGoals': typeof window.Goals.getGoals
                });
            }
            
            if (window.Tasks) {
                console.log('[PerhapsDebug] Tasks functions:', {
                    'addTask': typeof window.Tasks.addTask
                });
            }
            
            alert(`🔧 Estado de Módulos:\n\n${Object.entries(modules).map(([name, available]) => `${name}: ${available ? '✅' : '❌'}`).join('\n')}\n\nRevisa la consola para más detalles.`);
        },

        // Force render
        forceRender: () => {
            console.log('[PerhapsDebug] Forcing render...');
            if (window.Perhaps && window.Perhaps.renderIdeas) {
                window.Perhaps.renderIdeas();
                console.log('[PerhapsDebug] Render forced');
                alert('Render forzado ejecutado. Revisa la consola para más detalles.');
            } else {
                console.error('[PerhapsDebug] renderIdeas function not available');
                alert('Función renderIdeas no disponible');
            }
        }
    };
